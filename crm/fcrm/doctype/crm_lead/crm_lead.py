...
class CRMLead(Document):
	...
	def validate_mobile_no(self):
		"""
		Accept only a real phone number and store it in a clean, consistent form.

		See `crm.utils.phone.normalize_mobile_no` for the rules — Contact shares them.
		"""
		if not self.mobile_no:
			return

		self.mobile_no = normalize_mobile_no(self.mobile_no, self.meta.get_label("mobile_no"))
	...
	def assign_agent(self, agent):
		...
		assign_to_agent("CRM Lead", self.name, agent)
