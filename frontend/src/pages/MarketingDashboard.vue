  ...
  <div class="flex h-full flex-col overflow-hidden">
    <LayoutHeader>
      <template #left-header>
        <Breadcrumbs :items="[{ label: __('Dashboards'), route: { name: 'Dashboard' } }, { label: __('Marketing'), route: { name: 'MarketingDashboard' } }]" />
      </template>
    </LayoutHeader>

    <div class="flex-1 overflow-y-auto pa-page">
      <div class="pa-page-h">
        <h1 class="pa-h1">{{ __('Marketing Dashboard') }} <span class="pa-live">{{ __('Live') }}</span></h1>
      </div>
      ...
      <template v-if="d">
        ...
        <div class="mb-3.5 grid grid-cols-2 gap-3.5 lg:grid-cols-4">
          <Tile :icon="LucideUpload" :title="__('Leads Uploaded — MTD')" :value="String(d.lead_volume.uploaded_mtd)">
            <template #foot>
          ...
          <Tile :icon="LucideLayers" :title="__('This Quarter')" :value="String(d.lead_volume.this_quarter)" :sub="__('leads uploaded')" />
          <Tile :icon="LucideCalendar" :title="__('This Year')" :value="String(d.lead_volume.this_year)" :sub="__('leads uploaded')" />
          <Tile :icon="LucidePhone" :title="__('Not Yet Contacted')" :value="String(d.lead_volume.not_contacted_7d)" :sub="__('uploaded > 7 days ago')" tone="red" />
        </div>

        <div class="mb-8 grid grid-cols-1 gap-3.5 lg:grid-cols-2">
          <Card :title="__('Leads by Source')">
            <div v-if="sourceTotal" class="pa-stacked">
              <div v-for="s in d.by_source" :key="s.label" :style="`width: ${pct(s.count, sourceTotal)}%; background: ${s.color}`" :title="s.label"></div>
            </div>
            <div v-for="s in d.by_source" :key="s.label" class="pa-legend-row">
        ...
        <div class="mb-3.5 grid grid-cols-2 gap-3.5 lg:grid-cols-4">
          <Tile :icon="LucideArrowLeftRight" :title="__('Converted to Deal — MTD')" :value="String(d.conversion.converted_mtd)" :sub="__('leads → deals')" tone="green" />
          <Tile :icon="LucidePercent" :title="__('Lead Conversion Rate')" :value="`${d.conversion.conversion_rate}%`" :sub="__('converted / uploaded')" tone="green" />
          <Tile :icon="LucideClock" :title="__('Avg Days to Conversion')" :value="`${d.conversion.avg_days_to_conversion} d`" :sub="__('upload → converted')" />
          <Tile :icon="LucideMegaphone" :title="__('Marketing Contribution')" :value="`${d.conversion.contribution_pct}%`" :sub="__('of open pipeline')" />
        </div>
        ...
        <div class="mb-4 grid grid-cols-1 gap-3.5 sm:grid-cols-3">
          <Tile :icon="LucideIndianRupee" :title="__('Revenue — Marketing-Sourced Accounts')" :value="fmtINR(d.repeat.revenue_from_mktg)" :sub="__('lifetime order value')" tone="green" />
          <Tile :icon="LucideRepeat" :title="__('Now Repeat Buyers')" :value="`${d.repeat.now_repeat_buyers} / ${d.repeat.total_mktg_accounts}`" :sub="__('marketing accounts with >1 order')">
            <template #foot>
              <div class="pa-minitrack">
          ...
          <Tile :icon="LucidePercent" :title="__('Contribution % to Total Revenue')" :value="`${d.repeat.contribution_pct}%`" :sub="__('marketing-sourced / total')">
            <template #foot>
              <div class="pa-minitrack">
