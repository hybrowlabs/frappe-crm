...
const stageCta = computed(() => {
  ...
  // A deal the tech team sent back parks in Request for New Info until the salesperson
  // answers the questions inside the Qualified stage form — the CTA reads as a resume,
  // not a fresh start.
  if (
    status === 'Request for New Info' ||
  ...
  if (['Tech Evaluation', 'Retrial'].includes(status)) {
...
const STAGE_MODALS = {
  ...
  'Request for New Info': 'showInitiateTrialModal',
...
function onStageAction() {
  ...
  if (status === 'Proposal/Quotation') {
    router.push({
      name: 'New Quotation',
      query: { customer: organization.value?.erpnext_customer || undefined, deal: props.dealId },
    })
    return
  }
...
// and target status. Retrial and Request for New Info are optional branches, so they may
// be skipped.
const SKIPPABLE_STAGES = ['Retrial', 'Request for New Info']
...
async function triggerStatusChange(value) {
  ...
  if (singleStep) {
    ...
    } else if (['Tech Evaluation', 'Retrial'].includes(current)) {
...
function saveLostReason(lostFields) {
  saveChanges(
    { ...pendingLostChanges.value, ...lostFields },
    { onSuccess: () => sections.reload() },
  )
